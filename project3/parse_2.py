...
trace_file= open('project32.tr','r')
count = 0
variant1_count = 0
variant2_count = 0
pcount1 = 0
...
for row in trace_file:
	...
	if row[0] == '-' and row[7] == '3' and row[2] == source_1 and row[3] == destination_1 and row[8] == source_addr1 and row[9] == dest_addr1:
	...
	if row[0] == '-' and row[7] == '3' and row[2] == '4' and row[3] == '1' and row [8] == '4.0' and row[9] == '5.0':
		if row[11] not in latency_dict1:
			latency_dict1[row[11]] = float(row[1])

	if row[0] == '-' and row[7] == '3' and row[2] == source and row[3] == '5' and row[8] == '4.0' and row[9] == '5.0':
	...
	if row[0] == 'd' and row[4] == "tcp" and row[7] == '3':
		variant2_count = variant2_count + 1
		#print "Drops from this variant2:",variant2_count
...
with open("throughput1.txt", "a") as variant1_throughput:
    variant1_throughput.write(str(throughput_1)+'\n')
with open("throughput2.txt", "a") as variant2_throughput:
    variant2_throughput.write(str(throughput_2)+'\n')
...
with open("latency1.txt","a") as variant1_latency:
    variant1_latency.write(str(latency)+'\n')
with open("latency2.txt","a") as variant2_latency:
    variant2_latency.write(str(latency_1)+'\n')
